...
## 目录
  ...
  - [1.Redux: Extracting Presentational Components (Todo, TodoList)](#1redux-extracting-presentational-components-todo-todolist)
  - [2.Redux: Extracting Presentational Components (AddTodo, Footer, FilterLink)](#2redux-extracting-presentational-components-addtodo-footer-filterlink)
  - [3.Redux: Extracting Container Components (FilterLink)](#3redux-extracting-container-components-filterlink)
  - [4.Redux: Extracting Container Components (VisibleTodoList, AddTodo)](#4redux-extracting-container-components-visibletodolist-addtodo)
  - [5.Redux: Passing the Store Down Explicitly via Props](#5redux-passing-the-store-down-explicitly-via-props)
  - [6.Redux: Passing the Store Down Implicitly via Context](#6redux-passing-the-store-down-implicitly-via-context)
  - [7.Redux: Passing the Store Down with <Provider> from React Redux](#7redux-passing-the-store-down-with-provider-from-react-redux)
  - [8. Redux: Generating Containers with connect() from React Redux (VisibleTodoList)](#8-redux-generating-containers-with-connect-from-react-redux-visibletodolist)
  - [9. Redux: Generating Containers with connect() from React Redux (AddTodo)](#9-redux-generating-containers-with-connect-from-react-redux-addtodo)
  - [10. Redux: Generating Containers with connect() from React Redux (FilterLink)](#10-redux-generating-containers-with-connect-from-react-redux-filterlink)
  - [11.Redux: Extracting Action Creators](#11redux-extracting-action-creators)



## 1.Redux: Extracting Presentational Components (Todo, TodoList)

1

上次的内容中，我们完成了一个简单的todo app，里面我们可以做一个正常todo app可以做的一些标准操作，比如新增todo...比如切换todo的完成状态...比如切换todo list的过滤器。

我们把这些所有的内容都写在了todo app这个组件当中，比如在头部，我们有个input负责输入todo的text，按钮负责触发add todo action。列表的li上有onclick dispatch toggleTodo action，最下面是FilterLink负责切换visibility filter状态。
...
第一个修改我会把key这个属性去掉，因为只有一个array里才需要这个属性，key这个属性应该写在使用todo 组件的地方。
...
## 2.Redux: Extracting Presentational Components (AddTodo, Footer, FilterLink)
...
最后我想重构的组件是foot component，他包括了底部的3个filter链接。

我创建一个新stateless 组件，名叫footer。

目前还不太清楚需要什么属性，所以先声明属性，把复制过来的代码贴进render返回值中。

从代码中可以看到filter link组件需要visibility filter属性，所以我把他定义成为属性。
...
目前的filter link组件看起来并不直接处理业务，但他也需要current filter属性来决定如何渲染当前是否激活的状态。所以，有点矛盾的地方是，filter link真的是那么希望成为一个presentational component，因为他里面还是包含一点业务的逻辑。

他所应该做的事情就是在点击发生的时候dispatch set visibility filter action。
...
原来命名为FilterLink的组件将会成为一个container component，这次我把它声明成class类型的组件。

他的render中显示link组件，从store上获取link组件属性所需的值。

他需要得到他自己的props，和state，这里说的state并不是指react setState的state，而是Redux中store的getState里的state。
...
## 4.Redux: Extracting Container Components (VisibleTodoList, AddTodo)
...
这个优化使数据的流向变得依赖性没有那么强了，这使得开发者在使用FilterLink组件的时候不用关心那些为他内部使用的其他组件而需要的额外的数据。
...
所有的container component实际上来模式都差不多，他们的作用无非就是将presentation component和redux store的数据连接起来，将presentation component所需要的数据和动作传给他。
...
最后，在之前我们把add todo写成了一个presentational component，但是现在我要为他平反了。我会把on add click中的dispatch部分拷贝到add todo的内部。
...
在之前的使用中，我们之所以可以store这个变量，是因为它在我们的环境中是一个全局变量。
...
虽然我们在没有container component之前，要把所有数据逐层下传强一点点，但还是非常不方便。不过别担心，这是有一个更好的解决办法的，现在我们先这样做。
...
在provider的定义中会使用context，来保证凡是在provider环境内的子组件都可以使用到指定的store对象。
...
为了处理onTodoClick我创建另外一个转换函数，叫做mapDispatchToProps，顾名思义，这个属性里需要用到store.dispatch这个方法，所以，把这个dispatch这个方法作为mapDispatchToProps的参数传入。
...
## 10. Redux: Generating Containers with connect() from React Redux (FilterLink)
...
## 11.Redux: Extracting Action Creators
